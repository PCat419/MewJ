...
# 缓存：tile_name -> base64 data URI。每种牌图只写入报告 CSS 一次，
# 正文通过 class 复用，避免同一段 base64 随每个 <img> 重复数百次。
_TILE_BASE64_CACHE: dict[str, str] = {}


def _tile_base64(name: str) -> Optional[str]:
    """Return a base64 data URI for a tile PNG, or None if missing."""
    if not name:
        return None
    key = str(name).strip().lower()
    if key in _TILE_BASE64_CACHE:
        return _TILE_BASE64_CACHE[key]
...
def _tile_src(name: Optional[str], tile_base: str) -> Optional[str]:
    """Return base64 data URI for a tile PNG (tile_base is ignored)."""
    return _tile_base64(name)


def _tile_sprite_css() -> str:
    """Build one embedded CSS rule per tile for a compact standalone report."""
    rules = []
    for path in sorted(_TILE_DIR.glob("*.png")):
        key = path.stem.strip().lower()
        # Tile asset names are mpsz identifiers; keep the selector constrained.
        if not key or not all(ch.isalnum() or ch in "_-" for ch in key):
            continue
        src = _tile_base64(key)
        if src:
            rules.append(f'.tile-{key}{{background-image:url("{src}")}}')
    return "\n".join(rules)


def _tile_img(name: Optional[str], tile_base: str, extra_class: str = "") -> str:
    """Render one tile using the report's deduplicated embedded CSS assets."""
    if not name:
        return ""
    key = str(name).strip().lower()
    src = _tile_base64(key)
    cls = f"tile-img tile-{key} {extra_class}".strip()
    if src:
        return (
            f'<span class="{_esc(cls)}" role="img" aria-label="{_esc(name)}" '
...
def render_classic_html(
    ...
    tile_sprite_css = _tile_sprite_css()
    sections = []
    side_groups = []
    ...
    tenhou_bundle_json = ""
    legacy_tenhou_json = ""
    # Replay is provided by Tenhou's official online HTML5 viewer. The report
    # sends one kyoku at a time and reloads only when the kyoku changes.
    viewer_url = "https://tenhou.net/5/"
    ...
    <aside class="player-pane">
      <div class="player-head">
        ...
        <span class="player-online">在线 · 按小局切换</span>
      </div>
    ...
    <aside class="player-pane">
      ...
      <script type="application/json" id="legacy-tenhou">{legacy_tenhou_json}</script>
    </aside>"""
    else:
        player_pane = """
...
.player-online {{ color:var(--muted); font-size:.75rem; }}
iframe.tenhou {{
...
.tile-img {{
  display:inline-block; flex:0 0 auto; width:36px; height:50px;
  background-position:center; background-repeat:no-repeat; background-size:contain;
  vertical-align:middle;
  margin:0 1px 0 0; border-radius:3px; background-color:#f8fafc;
...
function viewerUrl(payload, kyokuIndex) {{
  var query = '?tw=' + viewerSeat + '&ts=0';
  if (kyokuIndex != null) query += '&_k=' + kyokuIndex;
  return viewerBase + query + '#json=' + encodeURIComponent(JSON.stringify(payload));
}}
...
function applyViewerSeek(ki, tjNum) {{
  ...
  // Online viewer reloads at kyoku granularity. Turn changes within the same
  // kyoku only expand the analysis card and preserve current replay position.
  var key = String(ki);
  ...
  frame.src = viewerUrl(payload, ki);
}}
...
(function () {{
  ...
  lastViewerKey = null;
  var legacyEl = document.getElementById('legacy-tenhou');
  var viewerFrame = document.getElementById('tenhou-player');
  if (legacyEl && viewerFrame) {{
    try {{ viewerFrame.src = viewerUrl(JSON.parse(legacyEl.textContent), null); }} catch (e) {{}}
  }}
